...
// CORS + JSON (pozor: webhook má raw body nižšie)
...
// Fixná suma 10 € – vytvor Checkout Session
app.post('/api/checkout', async (req,res)=>{
  try{
    const { message = "" } = req.body || {};
    const clean = String(message).replace(/\s+/g," ").replace(/https?:\/\/\S+/g,"[link]").slice(0,240);
...
app.post('/api/webhook',
  ...
  async (req,res)=>{
    ...
    if (event.type === 'checkout.session.completed'){
      ...
      }catch(e){ console.error('process webhook error', e); }
    }
